...
import com.cn.test.base.ObjectBeanUtils;
	...
	public abstract class GenericDaoHibernate {
		 ...
		 */
		@SuppressWarnings("unchecked")
		protected Pagination find(Finder finder, Integer pageNo, Integer pageSize) {
			Pagination p = null;
			...
			if(pageNo!=null){
				...
				p = new Pagination(1, totalCount, totalCount);
			}
			if (totalCount < 1) {
		...
		protected Pagination findByCriteria(Criteria crit, int pageNo, int pageSize) {
			...
			try {
				orderEntries = (List) ObjectBeanUtils
						.getFieldValue(impl, ORDER_ENTRIES);
				ObjectBeanUtils.setFieldValue(impl, ORDER_ENTRIES, new ArrayList());
			} catch (Exception e) {
			...
			try {
				ObjectBeanUtils.setFieldValue(impl, ORDER_ENTRIES, orderEntries);
			} catch (Exception e) {
				throw new RuntimeException(
		...
		protected int countQueryResult(Finder finder) {
			//System.out.println("xxxxxxxxxxxxxxxx  "+finder.getRowCountHql());
